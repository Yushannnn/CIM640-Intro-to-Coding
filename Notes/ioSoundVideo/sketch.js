...
function setup() {
 ...
 // select("#mycancas").style("margin", "0 auto");
...
//Do not play sounds untriggerd in the draw, triggerd on/off, mousepressed is perfect way
  //soundFile.play();
...
class Interfaces{
  constructor(tempX, tempY, tempS, tempC, tempName){
    ...
    //this grey over show when mousepressed,grey fill with 50% opatcity
  ...
  check(mX, mY){
     if(mX > this.x && mX < this.x+this.size && mY > this.y &&
       mY < this.y+this. size){
        this.overlay = true;
        return true;
     }else{
